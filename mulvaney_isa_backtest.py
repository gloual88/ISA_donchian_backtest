...
ISA_DEF = {
    ...
    "농산물(H)":     ("DBA",   "hed", "원자재"),
    "미국달러선물":  ("KRW=X", "krw", "통화"),
    # ── 업종 ETF 확장 (2026-06-06) ──
    # 미국 업종: ISA 매매 가능한 건 사실상 반도체뿐(한국상장 KODEX 미국반도체).
    #   백테스트 프록시 SOXX(환노출). 다른 미국 GICS 업종은 한국상장 부재로 제외.
    "미국반도체":    ("SOXX",  "unh", "주식"),
    # 한국 업종: KRW 네이티브(한국상장). 가능한 한 넓게(2007~2018 상장).
    "한국반도체":    ("091160.KS", "krw", "주식"),
    "한국IT":        ("139260.KS", "krw", "주식"),
    "한국2차전지":   ("305540.KS", "krw", "주식"),
    "한국자동차":    ("091180.KS", "krw", "주식"),
    "한국은행":      ("091170.KS", "krw", "주식"),
    "한국증권":      ("102970.KS", "krw", "주식"),
    "한국헬스케어":  ("266420.KS", "krw", "주식"),
    "한국바이오":    ("244580.KS", "krw", "주식"),
    "한국에너지화학": ("117460.KS", "krw", "주식"),
    "한국철강":      ("117680.KS", "krw", "주식"),
    "한국건설":      ("117700.KS", "krw", "주식"),
    "한국운송":      ("140710.KS", "krw", "주식"),
    # 266360=KODEX IT소프트웨어(한국IT와 중복) 제외. 아래 두 라벨은 yf 검증 후 정정.
    "한국필수소비재": ("139280.KS", "krw", "주식"),  # TIGER 필수소비재
    "한국경기소비재": ("139290.KS", "krw", "주식"),  # TIGER 경기소비재
}
...
def build_krw_panel():
    ...
    for label, (src, mode, _sec) in ISA_DEF.items():
        if mode == "krw" and src == "^KS200":
            ...
            # 일반 한국상장 ETF(KRW 네이티브) — us 배치에서 다운로드됨, 환율 미적용
